...
export const verifyUser = async (req, res, next) => {
    try {
        ...
        if (!exist) return res.status(500).send({ error: "Can't find User" });
        next();
    } catch (error) {
...
export const register = async (req, res) => {
    const { username, password, email, profile, college } = req.body;
    if (password) {
        bcrypt
            .hash(password, 10)
            .then((hashedPassword) => {
...
export const login = async (req, res) => {
    ...
    try {
        ...
        if (!user) {
            res.status(500).send({ err: "Username or Email not found" });
            return;
        }
        ...
        bcrypt.compare(password, user.password).then((passwordCheck) => {
            if (!passwordCheck) {
                ...
                const token = jwt.sign(
                    {
                        userId: user._id,
                        username: user.username, // Always use the username for JWT signing
                    },
                    "secret",
                    { expiresIn: "24h" }
                );
